...
class _TransactionPageState extends State<TransactionPage> {
          ...
          children: [
            ...
            Row(
              children: [
                SizedBox(
                  width: 180,
                  child: TextField(
                    decoration: const InputDecoration(labelText: 'Nomor Kupon', border: OutlineInputBorder()),
                    onChanged: (val) {
                      Provider.of<TransaksiProvider>(context, listen: false).setFilterTransaksi(nomorKupon: val);
                    },
                  ),
                ),
                ...
                SizedBox(
                  ...
                  child: TextField(
                    ...
                    onChanged: (val) {
                      Provider.of<TransaksiProvider>(context, listen: false).setFilterTransaksi(satker: val);
                    },
                  ),
                ),
                const SizedBox(width: 12),
                SizedBox(
                  ...
                  child: DropdownButtonFormField<int>(
                    ...
                    onChanged: (val) {
                      Provider.of<TransaksiProvider>(context, listen: false).setFilterTransaksi(jenisBbmId: val);
                    },
                  ),
                ),
                const SizedBox(width: 12),
                SizedBox(
                  ...
                  child: DropdownButtonFormField<int>(
                    ...
                    items: List.generate(31, (i) => DropdownMenuItem(value: i+1, child: Text((i+1).toString()))),
                    onChanged: (val) {
                      Provider.of<TransaksiProvider>(context, listen: false).setFilterTransaksi(hari: val);
                    },
                  ),
                ),
                const SizedBox(width: 12),
                SizedBox(
                  ...
                  child: DropdownButtonFormField<int>(
                    ...
                    onChanged: (val) {
                      Provider.of<TransaksiProvider>(context, listen: false).setFilterTransaksi(bulan: val);
                    },
                  ),
                ),
                const SizedBox(width: 12),
                SizedBox(
                  ...
                  child: DropdownButtonFormField<int>(
                    ...
                    items: [2024,2025].map((e) => DropdownMenuItem(value: e, child: Text(e.toString()))).toList(),
                    onChanged: (val) {
                      Provider.of<TransaksiProvider>(context, listen: false).setFilterTransaksi(tahun: val);
                    },
                  ),
                ),
                const SizedBox(width: 12),
            ...
            Row(
              ...
              children: [
                ...
                ElevatedButton.icon(
                  onPressed: () => _showTambahTransaksiDialog(context, jenisBbm: 1),
                  icon: const Icon(Icons.add),
                  label: const Text('Tambah Pertamax'),
                ),
                ElevatedButton.icon(
                  onPressed: () => _showTambahTransaksiDialog(context, jenisBbm: 2),
                  icon: const Icon(Icons.add),
            ...
            const SizedBox(height: 16),
            const Text('Kupon Minus', style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  void _exportTransaksi() async {
    ...
    if (transaksi.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Tidak ada data transaksi untuk diexport.')));
      return;
    }
    ...
    if (fileBytes == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Gagal membuat file Excel.')));
      return;
    }
    final file = File(outputPath);
    await file.writeAsBytes(fileBytes, flush: true);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Export berhasil: $outputPath')));
  }

  Future<void> _showTambahTransaksiDialog(BuildContext context, {required int jenisBbm}) async {
  final dashboardProvider = Provider.of<DashboardProvider>(context, listen: false);
  final transaksiProvider = Provider.of<TransaksiProvider>(context, listen: false);
  // Filter kuponList sesuai jenisBbm
  final List<KuponEntity> kuponList = dashboardProvider.kuponList.where((k) => k.jenisBbmId == jenisBbm).toList();
  final List<String> kuponOptions = kuponList.map((k) => '${k.nomorKupon}/${k.bulanTerbit}/${k.tahunTerbit}/${k.namaSatker}').toList();
    final _formKey = GlobalKey<FormState>();
    final _tanggalController = TextEditingController(text: DateTime.now().toIso8601String().substring(0,10));
    String? _nomorKupon;
    double? _jumlahLiter;
    await showDialog(
      context: context,
              ...
              children: [
                ...
                Autocomplete<String>(
                  optionsBuilder: (TextEditingValue textEditingValue) {
                    return kuponOptions.where((option) => option.contains(textEditingValue.text));
                  },
                  onSelected: (value) {
                    _nomorKupon = value;
                  },
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                for (final k in kuponList) {
                  final formatLengkap = '${k.nomorKupon}/${k.bulanTerbit}/${k.tahunTerbit}/${k.namaSatker}';
                ...
                if (kupon.kuotaSisa < (_jumlahLiter ?? 0)) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Kuota tidak cukup!')));
                  return;
                }
  ...
  Widget _buildTransaksiTable(BuildContext context) {
    return Consumer<TransaksiProvider>(
      builder: (context, provider, _) {
        ...
        final transaksiList = transaksiListRaw.map((t) =>
          t is TransaksiModel
            ? t
            : TransaksiModel(
                purchasingId: t.purchasingId,
                kuponId: t.kuponId,
                nomorKupon: t.nomorKupon,
                kendaraanId: t.kendaraanId,
                satkerId: t.satkerId,
        ...
        return SingleChildScrollView(
          ...
          child: DataTable(
            ...
            rows: transaksiList.map((t) => DataRow(cells: [
              ...
              DataCell(Row(
                children: [
                  IconButton(
                    ...
                    onPressed: () => _showEditTransaksiDialog(context, t),
                  ),
                  IconButton(
                    ...
                    onPressed: () async {
                      ...
                      if (confirm == true) {
                        final transaksiProvider = Provider.of<TransaksiProvider>(context, listen: false);
                        await transaksiProvider.deleteTransaksi(t.purchasingId);
                      }
                    },
                  ),
                ],
  ...
  Future<void> _showEditTransaksiDialog(BuildContext context, TransaksiModel t) async {
    final transaksiProvider = Provider.of<TransaksiProvider>(context, listen: false);
    final _formKey = GlobalKey<FormState>();
    final _tanggalController = TextEditingController(text: t.tanggalTransaksi);
    final _jumlahController = TextEditingController(text: t.jumlahDiambil.toString());
              ...
              onPressed: () async {
                if (_formKey.currentState?.validate() ?? false) {
                  final transaksiEdit = TransaksiModel(
                    ...
                    jenisKuponId: t.jenisKuponId,
                    tanggalTransaksi: _tanggalController.text,
                    jumlahDiambil: double.tryParse(_jumlahController.text) ?? t.jumlahDiambil,
                    keterangan: t.keterangan,
                    createdAt: t.createdAt,
                    isDeleted: t.isDeleted,
                    deletedAt: t.deletedAt,
      ...
      builder: (context, provider, _) {
        ...
        return SingleChildScrollView(
          ...
          child: DataTable(
            ...
            rows: minus.map((m) => DataRow(cells: [
              DataCell(Text(m['nomor_kupon'].toString())),
              DataCell(Text(m['kuota_sisa'].toString())),
              DataCell(Text(m['jenis_bbm_id'].toString())),
              DataCell(IconButton(icon: const Icon(Icons.download), onPressed: () {/* TODO: Export kupon minus */})),
            ])).toList(),
          ),
        );
      },
